 ...
 */
/* eslint-disable no-bitwise -- SHA-256 requires bit ops */
...
export function sha256HexFromUtf8(input) {
  const message = utf8Bytes(input);
  const bitLength = message.length * 8;
  const withPadding = new Uint8Array((message.length + 9 + 63 & ~63) >>> 0);
  withPadding.set(message);
  withPadding[message.length] = 0x80;
  const view = new DataView(withPadding.buffer);
  view.setUint32(withPadding.length - 4, bitLength >>> 0, false);
  // High 32 bits of length stay 0 for inputs under 512 MiB (our use case).
